...
########################################################################
[ -z "$CI_TIME" ] || echo "`date`: Build filemq from local source"

(android_build_verify_so "libfilemq.so" "${DEPENDENCIES[@]}" &> /dev/null) || {
    (
        CONFIG_OPTS=()
        [ "${CI_CONFIG_QUIET}" = "yes" ] && CONFIG_OPTS+=("--quiet")
        CONFIG_OPTS+=("${ANDROID_BUILD_OPTS[@]}")
        ...
        android_build_library "LIBFILEMQ" "${PROJECT_ROOT}"
    ) || exit 1
}

##
# Verify shared libraries in prefix
for library in "libfilemq.so" "${DEPENDENCIES[@]}" ; do
    android_build_verify_so "${library}"
done

android_build_verify_so "libfilemq.so" "${DEPENDENCIES[@]}"
