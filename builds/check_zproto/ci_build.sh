...
docker run -e GSL_BUILD_DIR=/code/src -e BUILD_DIR=/code/src \
    -v "$REPO_DIR":/code zeromqorg/zproto -zproject:1 -q fmq_msg.xml
docker run -e GSL_BUILD_DIR=/code/src -e BUILD_DIR=/code/src \
    -v "$REPO_DIR":/code zeromqorg/zproto -zproject:1 -q fmq_server.xml
docker run -e GSL_BUILD_DIR=/code/src -e BUILD_DIR=/code/src \
    -v "$REPO_DIR":/code zeromqorg/zproto -zproject:1 -q fmq_client.xml
