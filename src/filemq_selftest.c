...
// Tests for draft public classes:
    { "fmq_msg", fmq_msg_test, false, true, NULL },
    { "fmq_server", fmq_server_test, false, true, NULL },
    { "fmq_client", fmq_client_test, false, true, NULL },
...
test_runall (bool verbose)
{
    ...
    for (item = all_tests; item->testname; item++) {
        ...
        if (!item->subtest)
            item->test (verbose);
#ifdef FILEMQ_BUILD_DRAFT_API // selftest is still in draft
        else
            filemq_private_selftest (verbose, item->subtest);
#endif // FILEMQ_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("filemq_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running filemq test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef FILEMQ_BUILD_DRAFT_API // selftest is still in draft
        else
            filemq_private_selftest (verbose, test->subtest);
#endif // FILEMQ_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
